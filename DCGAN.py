...
def generator(z, reuse=None):
	with tf.variable_scope('gen',reuse=reuse):
		keep_prob=0.2
		hidden1=tf.layers.dense(inputs=z,units=4*4*3,activation=tf.nn.leaky_relu)
		...
		hidden3=tf.layers.conv2d_transpose(inputs=dropout_2, kernel_size=3, filters=64, strides=1, padding='same', activation=tf.nn.leaky_relu)
		dropout_3=tf.layers.dropout(hidden3, rate=keep_prob)
		hidden4=tf.layers.conv2d_transpose(inputs=dropout_3, kernel_size=3, filters=32, strides=1, padding='same', activation=tf.nn.leaky_relu)
		dropout_4=tf.layers.dropout(hidden2, rate=keep_prob)
		output=tf.layers.conv2d_transpose(inputs=dropout_4, kernel_size=3, filters=1, strides=1, padding='same', activation=tf.nn.sigmoid)
		return output
def discriminator(X, reuse=None):
    with tf.variable_scope('dis',reuse=reuse):
    	...
    	hidden2=tf.layers.conv2d(inputs=dropout_1, kernel_size=3, filters=64,strides=1, padding='same', activation=tf.nn.leaky_relu)
    	dropout_2= tf.layers.dropout(inputs=hidden2, rate=0.2)
    	hidden3=tf.layers.conv2d(inputs=dropout_2, kernel_size=3, filters=64,strides=1, padding='same', activation=tf.nn.leaky_relu)
    	dropout_3= tf.layers.dropout(inputs=hidden3, rate=0.2)
    	x_flat = tf.contrib.layers.flatten(dropout_3)
...
with tf.Session() as sess:
	...
	for epoch in range(epochs):
		...
		for i in range(num_batches):
			...
			print("Starting d_trainer")
			_=sess.run(D_trainer, feed_dict={real_images:batch_images, z:batch_z})
			print("Done with d_trainer")
