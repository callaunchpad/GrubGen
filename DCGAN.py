...
def generator(z, reuse=None):
	with tf.variable_scope('gen',reuse=reuse):
		keep_prob=0.2
		momentum = 0.9
		hidden1=tf.layers.dense(inputs=z,units=4*4*4,activation=tf.nn.leaky_relu)
		...
		hidden2=tf.layers.conv2d_transpose(inputs=full_reshape, kernel_size=5, filters=64, strides=2, padding='same', activation=tf.nn.leaky_relu)
		dropout_2=tf.layers.dropout(hidden2, rate=keep_prob)
		batch_norm2 = tf.contrib.layers.batch_norm(dropout_2, decay=momentum)
		hidden3=tf.layers.conv2d_transpose(inputs=batch_norm2, kernel_size=5, filters=64, strides=2, padding='same', activation=tf.nn.leaky_relu)
		dropout_3=tf.layers.dropout(hidden3, rate=keep_prob)
		batch_norm3 = tf.contrib.layers.batch_norm(dropout_3, decay=momentum)
		hidden4=tf.layers.conv2d_transpose(inputs=batch_norm3, kernel_size=5, filters=64, strides=1, padding='same', activation=tf.nn.leaky_relu)
		dropout_4=tf.layers.dropout(hidden2, rate=keep_prob)
		batch_norm4 = tf.contrib.layers.batch_norm(dropout_4, decay=momentum)
		output=tf.layers.conv2d_transpose(inputs=batch_norm4, kernel_size=5, filters=1, strides=1, padding='same', activation=tf.nn.sigmoid)
		return output
