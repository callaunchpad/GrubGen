...
def discriminator(X, reuse=None):
    with tf.variable_scope('dis',reuse=reuse):
    	...
    	hidden1=tf.layers.conv2d(inputs=X, kernel_size=4, filters=512, strides=2, padding='same', activation=tf.nn.leaky_relu)
    	batch_norm1 = tf.contrib.layers.batch_norm(hidden1, decay=momentum)
    	hidden2=tf.layers.conv2d(inputs=batch_norm1, kernel_size=4, filters=256,strides=2, padding='same', activation=tf.nn.leaky_relu)
    	batch_norm2 = tf.contrib.layers.batch_norm(hidden2, decay=momentum)
    	hidden3=tf.layers.conv2d(inputs=batch_norm2, kernel_size=4, filters=128,strides=2, padding='same', activation=tf.nn.leaky_relu)
    	batch_norm3 = tf.contrib.layers.batch_norm(hidden3, decay=momentum)
...
reshaped_rgb.astype('float32').tofile('reshaped_rgb_first5')
img = Image.fromarray(reshaped_rgb, 'RGB')
img.show()
reshaped_rgb_last = gen_samples[epochs-1].reshape(64, 64, 3)
reshaped_rgb_last.astype('float32').tofile('reshaped_rgb_last5')
